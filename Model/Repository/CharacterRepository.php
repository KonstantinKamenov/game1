...
class CharacterRepository{
    ...
    public function addCharacter($user_id,$name,$class){
        ...
        $query = 'INSERT INTO `characters` (`owner_id`, `name`, `class`, `health`, `attack`, `movement_speed`)
                VALUES (:user_id, :name, :class, :health, :attack, :movement_speed)';
        
        $stmt = $conn->prepare($query);
        return $stmt->execute(['user_id'=>$user_id,'name'=>$name,'class'=>$class,'health'=>$template['health'], 'attack'=>$template['attack'], 'movement_speed'=>$template['speed']]);
    }
}